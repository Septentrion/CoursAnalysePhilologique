...
## Introduction : le fossé entre deux cultures
...
Avant de discuter des choix spécifiques à notre projet, rappelons la structure générale d'un problème d'apprentissage supervisé, pour avoir un vocabulaire commun.

### 1.1 Les trois ingrédients fondamentaux
...
### 1.2 La supervision : qu'est-ce qu'un « label » pour un manuscrit ?
...
Pour un manuscrit médiéval, la situation est radicalement différente :

- **L'annotation est lente.** Un paléographe expérimenté transcrit entre 1 000 et 3 000 mots par jour sur un manuscrit difficile. Pour constituer un dataset d'entraînement de 50 000 lignes, il faudrait des années-personnes de travail.

- **L'annotation est qualifiée.** Lire un manuscrit médiéval ne s'improvise pas. Il faut connaître le système d'écriture de l'époque, les conventions d'abréviation, les dialectes possibles, et souvent le contexte du texte pour lever les ambiguïtés. Les annotateurs doivent être formés.
...
```
Document (ensemble des pages)
    └── Page
         └── Région (colonne, marge, rubrique, illustration)
              └── Ligne de texte
                   └── Mot
...
### 2.1 Le niveau du trait (*stroke*)

Le trait est l'unité élémentaire du geste d'écriture : le mouvement continu du calame ou de la plume entre deux levées. C'est le niveau d'analyse de la **graphématique** (la science de l'écriture au niveau du geste) et de certains travaux en analyse d'écriture en ligne (*online handwriting*).

Dans le contexte des manuscrits médiévaux numérisés (images statiques, *offline handwriting*), travailler au niveau du trait est pratiquement impossible : il faudrait segmenter chaque geste dans l'image, ce qui nécessite une annotation d'une granularité extrême et des algorithmes de segmentation qui n'existent pas de façon robuste pour les écritures médiévales.

Ce niveau est mentionné ici pour mémoire, mais n'est pas une option réaliste pour notre projet.

### 2.2 Le niveau du caractère
...
**Avantages**
- Problème bien défini mathématiquement : classification multi-classes.
...
**Verdict :** le niveau du caractère isolé est adapté aux textes imprimés, pas aux manuscrits médiévaux. Il est abandonné au profit d'approches séquentielles.

### 2.3 Le niveau du mot
...
**Verdict :** le niveau du mot est une option théorique rarement implémentée en pratique. Il est généralement abandonné au profit du niveau de la ligne.

### 2.4 Le niveau de la ligne : le choix canonique de l'HTR
...
### 2.6 Synthèse : granularités et tâches associées
...
Considérons un passage abrégé dans un manuscrit du XIIIe siècle. Un expert en paléographie médiévale produit la transcription :

```
"En cel tems que li rois Artus regnoit"
```

Un second expert, de formation légèrement différente, produit :

```
"En kel tens ke li reis Artu regnout"
```

Les deux transcriptions sont défendables : la première normalise légèrement les formes dialectales, la seconde reproduit plus fidèlement les formes picardes du scribe hypothétique. Qui a raison ? Ni l'un ni l'autre, ou les deux à la fois. La question est mal posée.

La situation réelle est encore plus nuancée :
...
Le CER est **la métrique standard de la communauté HTR**. Il mesure le taux d'erreur au niveau du caractère.

**Définition formelle**

Le CER est calculé à partir de la **distance d'édition de Levenshtein** entre la transcription prédite et la transcription de référence. La distance de Levenshtein entre deux chaînes est le nombre minimal d'opérations élémentaires nécessaires pour transformer l'une en l'autre.

Les trois opérations élémentaires sont :
- **Substitution** : remplacer un caractère par un autre (*a* → *o*).
- **Insertion** : ajouter un caractère absent de la référence.
- **Suppression** (ou délétion) : supprimer un caractère présent dans la référence.
...
- $D$ = nombre de suppressions (délétions)
...
Référence : `"li rois Artus"`  
Prédiction : `"li rous Artus"`

La différence est un seul caractère : le *o* à la place du *i* dans *rois* → *rous*. C'est une substitution.

$$\text{CER} = \frac{1 + 0 + 0}{13} \approx 7.7\%$$

Référence : `"En cel tems"`  
Prédiction : `"En cel tes"`

Une suppression (le *m* final de *tems* a été omis).

$$\text{CER} = \frac{0 + 0 + 1}{11} \approx 9.1\%$$
...
### 4.3 Le BLEU adapté à l'HTR

Le score BLEU (*Bilingual Evaluation Understudy*), développé à l'origine pour l'évaluation de la traduction automatique, mesure le recouvrement de n-grammes entre une prédiction et une référence.

Un **n-gramme** est une séquence contiguë de $n$ éléments (caractères ou mots). Un 2-gramme de caractères (*bigrame*) dans le mot *texto* est par exemple : *te*, *ex*, *xt*, *to*.

Le score BLEU calcule la précision des n-grammes prédits par rapport à la référence, avec une pénalité pour les prédictions trop courtes (*brevity penalty*).

**Pourquoi le BLEU est moins utilisé en HTR que le CER**

Le BLEU a été conçu pour la traduction : une bonne traduction peut être formulée de plusieurs façons, et le BLEU mesure si les formulations probables se retrouvent dans la référence. En HTR, on ne cherche pas à paraphraser : on cherche à reproduire exactement le texte visible. Le BLEU récompense le recouvrement partiel là où le CER pénalise toute divergence. Pour l'évaluation de la fidélité de transcription, le CER est plus approprié.

Le BLEU peut être utile pour évaluer une étape de **post-correction par modèle de langage** (module NLP), où plusieurs formulations normalisées d'un même texte peuvent être équivalentes.

### 4.4 Les limites des métriques dans le contexte médiéval
